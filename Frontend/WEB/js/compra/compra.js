...
function addCompra() {
  ...
  function habilitarVistaDetalle(idCompra){
    const destinationURL = `http://127.0.0.1:5500/Frontend/WEB/comprasDetail.html?idCompra=${idCompra+1}`;
    window.location.href = destinationURL;
  }

  function listCompras() {
        const apiUrl = 'https://localhost:7084/api/Compra';
        fetch(apiUrl)
        .then((response) => {
            if (!response.ok) {
